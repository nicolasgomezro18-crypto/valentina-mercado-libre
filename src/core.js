...
const SalesCore = (() => {
  ...
  function filter(rows,f){return rows.filter(r=>(!f.seller||r.sellerId===f.seller)&&(!f.account||r.accountId===f.account)&&(!f.status||r.status===f.status)&&(!(f.from||f.to)||validDate(r.date))&&(!f.from||r.date>=f.from)&&(!f.to||r.date<=f.to)&&(!f.settlement||(f.settlement==='review'?settlementReview(r):f.settlement==='yes'?!!r.settlementId:!r.settlementId&&!settlementReview(r)))&&(!f.search||normalize([r.reference,r.productName,r.sellerName,r.accountName].join(' ')).includes(normalize(f.search))));}
  ...
  return {normalize,amount,validDate,parseDate,total,extract,filter,summarize,csv,dashboard,settlementReview};
})();
